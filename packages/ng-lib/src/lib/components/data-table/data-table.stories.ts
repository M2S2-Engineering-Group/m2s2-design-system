...
@Component({
  ...
  styles: [
    `
      ...
      .demo-tbl tbody tr:last-child {
        border-bottom: none;
      }
      .demo-tbl tbody tr:hover {
        background: var(--color-surface-raised);
      }
      .demo-tbl td {
        padding: 10px 12px;
  ...
  template: `
    ...
    >
      <table class="demo-tbl">
        <thead>
          <tr>
            @if (colVisibility()["date"]) {
              <th m2s2ResizableCol class="col-date">Date</th>
            }
            @if (colVisibility()["name"]) {
              <th m2s2ResizableCol class="col-name">Name</th>
            }
            @if (colVisibility()["email"]) {
              <th m2s2ResizableCol class="col-email">Email</th>
            }
            @if (colVisibility()["engagement"]) {
              <th m2s2ResizableCol class="col-eng">Engagement</th>
            }
            @if (colVisibility()["status"]) {
              <th m2s2ResizableCol class="col-status">Status</th>
            }
          </tr>
        </thead>
        <tbody>
          @if (filtered().length === 0) {
            <tr>
              <td [attr.colspan]="visibleColCount()" class="tbl-empty">
                No matching results.
              </td>
            </tr>
          }
          @for (row of filtered(); track row.email) {
            <tr>
              ...
              @if (colVisibility()["engagement"]) {
                <td class="muted">{{ row.engagement }}</td>
              }
              @if (colVisibility()["status"]) {
                <td>
                  <span
