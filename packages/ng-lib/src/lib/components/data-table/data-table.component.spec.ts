...
describe('DataTableComponent', () => {
  ...
  it('marks the active status pill with dt-pill--active', async () => {
    await render(DataTableComponent, {
      inputs: { totalCount: 10, statuses: ['all', 'active'], statusFilter: 'active' },
    });
    expect(screen.getByRole('button', { name: 'active' })).toHaveClass('dt-pill--active');
    expect(screen.getByRole('button', { name: 'All' })).not.toHaveClass('dt-pill--active');
  });
  ...
  it('emits searchChange when the user types in the search input', async () => {
    ...
    fireEvent.input(screen.getByRole('searchbox'), { target: { value: 'hello' } });
  ...
  it('toggles the column panel when the Columns button is clicked', async () => {
    const { fixture } = await render(DataTableComponent, {
      inputs: {
        totalCount: 10,
        columnDefs: [{ key: 'name', label: 'Name' }, { key: 'email', label: 'Email' }],
