...
describe("FooterComponent", () => {
  ...
  it("renders the brand name and current year in the copyright line", async () => {
    ...
    expect(
      screen.getByText(new RegExp(`${currentYear}.*Acme Corp`)),
    ).toBeInTheDocument();
  });
    ...
    await render(FooterComponent, {
      inputs: {
        config: {
          brandName: "Acme",
          links: [
            { type: "email", href: "mailto:hello@acme.com", label: "Email" },
          ],
        },
      },
    });
  ...
  it("renders the build version, stripped of build metadata, when present", async () => {
    await render(FooterComponent, {
      inputs: {
        config: { brandName: "Acme", links: [], buildVersion: "2.9.1+8f3a1c9" },
      },
    });
