import { Directive, ElementRef, Renderer2, OnInit, DestroyRef, inject, input } from '@angular/core';
  ...
  ngOnInit(): void {
    ...
    fromEvent<PointerEvent>(handle, 'pointerdown').pipe(
      switchMap(start => {
        ...
        const overhead = parseFloat(styles.paddingLeft)
        ...
        return fromEvent<PointerEvent>(document, 'pointermove').pipe(
          map(move => {
            const borderBoxWidth = Math.max(this.minWidth(), startWidth + move.clientX - startX);
