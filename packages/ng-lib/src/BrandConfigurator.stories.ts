...
@Component({
      ...
      <header class="config-header">
        ...
        <div class="config-presets">
          ...
          @for (theme of themes; track theme.key) {
            <button class="preset-btn" [class.preset-btn--active]="activePreset === theme.key"
                    (click)="applyPreset(theme.key)">
              <span class="preset-swatch"
                    [style.background]="theme.gradientBrand135"></span>
              {{ theme.label }}
            </button>
          }
      ...
      <div class="config-body">
            ...
            @for (t of primaryTokens; track t.cssVar) {
              <div class="token-row">
                <div class="token-swatch-wrap">
                  <input type="color" class="color-picker" [ngModel]="getValue(t.cssVar)"
        ...
        <section class="token-section">
          ...
          <p class="section-note">
            Gradients are built from the start and end colors below.
            The hex values here define the gradient stops — the full
  ...
  styles: [`
    ...
    .preset-btn {
      ...
      &:hover { border-color: var(--color-primary); color: var(--color-on-surface); }
      &--active { border-color: var(--color-primary); background: var(--color-primary-dim); color: #fff; }
      &--reset { margin-left: 8px; border-style: dashed; }
    }
    .preset-swatch {
      width: 14px;
      height: 14px;
...
class BrandConfiguratorComponent implements OnInit {
  ...
  readonly primaryTokens: ColorToken[] = [
    ...
    { label: 'Primary Vivid', cssVar: '--color-primary-vivid', defaultValue: '#8b5cf6' },
    { label: 'Primary Dim',   cssVar: '--color-primary-dim',   defaultValue: '#5b21b6' },
    { label: 'On Primary',    cssVar: '--color-on-primary',    defaultValue: '#1a0030' },
  ];

  readonly secondaryTokens: ColorToken[] = [
    { label: 'Secondary',     cssVar: '--color-secondary',     defaultValue: '#22d3ee' },
    { label: 'Secondary Dim', cssVar: '--color-secondary-dim', defaultValue: '#0891b2' },
    { label: 'On Secondary',  cssVar: '--color-on-secondary',  defaultValue: '#001a20' },
  ];
  ...
  readonly gradientTokens: GradientToken[] = [
    {
      ...
      defaultValue: 'linear-gradient(90deg, #3b0764 0%, #8b5cf6 50%, #22d3ee 100%)',
      startKey: 'brand90Start', endKey: 'brand90End',
    },
    {
      label: 'Brand Gradient (135°)',
      cssVar: '--gradient-brand-135',
      defaultValue: 'linear-gradient(135deg, #3b0764 0%, #22d3ee 100%)',
      startKey: 'brand135Start', endKey: 'brand135End',
    },
    {
      label: 'Navbar Gradient',
      cssVar: '--gradient-nav-bar',
      defaultValue: 'linear-gradient(90deg, rgba(64,6,144,1) 0%, rgba(0,212,255,1) 100%)',
      startKey: 'navStart', endKey: 'navEnd',
    },
  ];

  gradientStops: Record<string, string> = {
    brand90Start: '#3b0764', brand90End: '#22d3ee',
    brand135Start: '#3b0764', brand135End: '#22d3ee',
    navStart: '#400690', navEnd: '#00d4ff',
  };
  ...
  ngOnInit(): void {
    for (const t of [...this.primaryTokens, ...this.secondaryTokens]) {
      const current = getComputedStyle(document.documentElement).getPropertyValue(t.cssVar).trim();
      this.values[t.cssVar] = current || t.defaultValue;
    }
    for (const g of this.gradientTokens) {
      const current = getComputedStyle(document.documentElement).getPropertyValue(g.cssVar).trim();
  ...
  onGradientChange(g: GradientToken, stop: 'start' | 'end', color: string): void {
    ...
    const grad = g.cssVar === '--gradient-nav-bar'
  ...
  applyPreset(key: string): void {
    ...
    const extractFirst = (grad: string) => grad.match(/#[0-9a-f]{6}/i)?.[0] ?? '#000000';
    const extractLast  = (grad: string) => grad.match(/#[0-9a-f]{6}/gi)?.slice(-1)[0] ?? '#ffffff';
  ...
  reset(): void {
    const props = [
      '--color-primary', '--color-primary-vivid', '--color-primary-dim', '--color-on-primary',
      '--color-secondary', '--color-secondary-dim', '--color-on-secondary',
      '--gradient-brand', '--gradient-brand-135', '--gradient-nav-bar',
    ];
