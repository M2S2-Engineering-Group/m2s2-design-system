const commitAnalyzer = ['@semantic-release/commit-analyzer', {
  preset: 'angular',
  releaseRules: [
    { type: 'feat',     scope: 'utils', release: 'minor' },
    { type: 'fix',      scope: 'utils', release: 'patch' },
    { type: 'perf',     scope: 'utils', release: 'patch' },
    { type: 'refactor', scope: 'utils', release: 'patch' },
    { breaking: true,   scope: 'utils', release: 'major' },
  ],
}];
...
const git = ['@semantic-release/git', {
  ...
  message: 'chore(release): @m2s2/utils v${nextRelease.version} [skip ci]',
}];
