...
const SERIES_COLORS = ['--color-primary', '--color-secondary', '--color-warning', '--color-error'];
...
export function buildTimeSeriesChartConfig(
  ...
  const seriesNames = Array.from(new Set(points.flatMap((p) => Object.keys(p.values))));
  ...
  return {
    ...
    options: {
      ...
      scales: { x: sharedScaleStyle(), y: { ...sharedScaleStyle(), beginAtZero: true } },
      plugins: {
        legend: {
 ...
 */
export function buildRankedBarChartConfig(items: RankedBarItem[]): ChartConfiguration {
  ...
  return {
    ...
    options: {
      ...
      responsive: true,
      maintainAspectRatio: false,
      scales: { x: { ...sharedScaleStyle(), beginAtZero: true }, y: sharedScaleStyle() },
