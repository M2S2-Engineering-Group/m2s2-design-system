...
describe("SubscribeForm — anon mode", () => {
  it("renders email and name inputs", () => {
    ...
    expect(wrapper.find('input[placeholder="your@email.com"]').exists()).toBe(
      true,
    );
    expect(
      wrapper.find('input[placeholder="Your name (optional)"]').exists(),
    ).toBe(true);
  });
  ...
  it("calls subscribeAnon with trimmed email and name on submit", async () => {
    ...
    await wrapper
      .find('input[placeholder="Your name (optional)"]')
      .setValue("Jane");
...
describe("SubscribeForm — auth mode", () => {
  ...
  it("calls unsubscribeAuth when unsubscribe is clicked", async () => {
    ...
    const wrapper = mount(SubscribeForm, {
      props: { mode: "auth", subscribeAuth, unsubscribeAuth },
    });
    await wrapper.find("button").trigger("click");
    await flushPromises();
