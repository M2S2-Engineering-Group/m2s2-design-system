...
describe("BlogEditor", () => {
  describe("renders fields", () => {
    it("renders the title input", () => {
      ...
      expect(wrapper.find('input[placeholder="Post title…"]').exists()).toBe(
        true,
      );
    });
    ...
    it("renders the slug input", () => {
      const wrapper = mountEditor();
      expect(wrapper.find('input[placeholder="post-slug"]').exists()).toBe(
        true,
      );
    });
  ...
  describe("Publish button state", () => {
    ...
    it("is disabled when summary is empty", async () => {
      const wrapper = mountEditor();
      await wrapper
        .find('input[placeholder="Post title…"]')
        .setValue("A title");
      expect(wrapper.find(".be-publish").attributes("disabled")).toBeDefined();
    });
    ...
    it("is enabled when title, summary, and content are all filled", async () => {
      ...
      await wrapper
        .find(
          'textarea[placeholder="Short description shown in blog listings…"]',
        )
        .setValue("A summary");
      await wrapper
      ...
      expect(
        wrapper.find(".be-publish").attributes("disabled"),
      ).toBeUndefined();
    });
  });
  ...
  describe("series dropdown", () => {
    ...
    it("hides Part and Total inputs when series is None", () => {
      const wrapper = mountEditor();
      expect(
        wrapper.find('input[placeholder="e.g. go-backend"]').exists(),
      ).toBe(false);
      expect(
        wrapper.find("label[for]").exists() && wrapper.text().includes("Part"),
      ).toBe(false);
    });
    ...
    it("shows series ID and title inputs when New series is selected", async () => {
      ...
      expect(
        wrapper.find('input[placeholder="e.g. Go Backend Series"]').exists(),
      ).toBe(true);
    });
  ...
  describe("series in published draft", () => {
    it("emits no series when series is None", async () => {
      ...
      await fillRequiredFields(wrapper);
      await wrapper.find(".be-publish").trigger("click");
      const draft = (wrapper.emitted("publish") as unknown[][])[0][0] as Record<
        string,
        unknown
      >;
    ...
    it("emits series when New series is selected and ID is filled", async () => {
      ...
      await wrapper.find("select").trigger("change");
      await wrapper
        .find('input[placeholder="e.g. go-backend"]')
        .setValue("go-backend");
      await wrapper
        .find('input[placeholder="e.g. Go Backend Series"]')
        .setValue("Go Backend Series");
      await wrapper.find(".be-publish").trigger("click");
      const draft = (wrapper.emitted("publish") as unknown[][])[0][0] as Record<
        string,
        unknown
