...
describe("Footer", () => {
  it("renders brand name in copyright", () => {
    const wrapper = mount(Footer, {
      props: { config: makeFooterConfig({ brandName: "Acme Corp" }) },
    });
  ...
  it("renders current year in copyright", () => {
    ...
    expect(wrapper.find(".footer-copy").text()).toContain(
  ...
  it("renders nothing in the social nav when links is empty", () => {
    const wrapper = mount(Footer, {
      props: { config: makeFooterConfig({ links: [] }) },
    });
  ...
  it("renders the build version, stripped of build metadata, when present", () => {
    const wrapper = mount(Footer, {
      props: { config: makeFooterConfig({ buildVersion: "2.7.0+8f3a1c9" }) },
    });
