...
describe("Chat", () => {
  ...
  describe("open / close toggle", () => {
    it("renders the toggle button and panel is hidden initially", () => {
      ...
      expect(wrapper.find('button[aria-label="Open chat"]').exists()).toBe(
        true,
      );
  ...
  describe("welcome message", () => {
    it("shows the welcome message as the first assistant message when opened with no prior conversation", async () => {
      ...
      await wrapper.find('button[aria-label="Open chat"]').trigger("click");
      expect(
        wrapper.find(".chat-message--assistant .chat-message__text").text(),
    ...
    it("does not show a welcome message when none is provided", async () => {
      const wrapper = mountChat();
      await wrapper.find('button[aria-label="Open chat"]').trigger("click");
      expect(wrapper.find(".chat-empty").text()).toBe(
    ...
    it("does not re-add the welcome message on subsequent opens once a conversation exists", async () => {
      ...
      expect(
        wrapper
          .findAll(".chat-message__text")
          .filter((el) => el.text() === "Hi there!"),
      ).toHaveLength(1);
    });
  });
  ...
  describe("title and subtitle", () => {
    ...
    it("renders the subtitle when no messages exist", async () => {
      ...
      await wrapper.find('button[aria-label="Open chat"]').trigger("click");
      expect(wrapper.find(".chat-header__subtitle").text()).toBe(
        "Ask me anything",
      );
    });
  });
  ...
  describe("error state", () => {
    it("shows an error message when sendMessage rejects", async () => {
      ...
      expect(wrapper.find(".chat-error").text()).toBe(
  ...
  describe("header content", () => {
    ...
    it("renders a plain string prop as text", async () => {
      ...
      await wrapper.find('button[aria-label="Open chat"]').trigger("click");
      expect(wrapper.find(".chat-header__extra").text()).toBe(
        "Now serving two personas",
      );
    });
    ...
    it("renders the headerContent slot, exactly as provided", async () => {
      ...
      expect(wrapper.find('[data-testid="custom-tab"]').text()).toBe(
        "Assistant",
      );
    });
    ...
    it("prefers the slot over the prop when both are provided", async () => {
      const wrapper = mount(Chat, {
        ...
        slots: {
          headerContent: '<span data-testid="slot-content">slot text</span>',
        },
      });
      await wrapper.find('button[aria-label="Open chat"]').trigger("click");
