...
describe('DataTable', () => {
  it('shows the empty message when totalCount is 0', () => {
    render(<DataTable totalCount={0} emptyMessage="Nothing here yet."><table /></DataTable>);
    expect(screen.getByText('Nothing here yet.')).toBeInTheDocument();
  });

  it('uses default empty message when none provided and totalCount is 0', () => {
    render(<DataTable totalCount={0}><table /></DataTable>);
    expect(screen.getByText('No data yet.')).toBeInTheDocument();
  });

  it('renders the search input when statuses are provided', () => {
    render(
      <DataTable totalCount={5} statuses={['all', 'active']} searchPlaceholder="Search items…">
        <table />
      </DataTable>
    );
  ...
  it('renders a pill for each status', () => {
    render(
      <DataTable totalCount={5} statuses={['all', 'active', 'inactive']} statusLabels={{ all: 'All', active: 'Active', inactive: 'Inactive' }}>
        <table />
      </DataTable>
    );
  ...
  it('falls back to the raw status string when no label is provided', () => {
    ...
    expect(screen.getByRole('button', { name: 'custom_status' })).toBeInTheDocument();
  });
