...
const pillStyle = (status: string): React.CSSProperties => ({
  ...
  fontWeight: 600,
  color: "#fff",
  background: STATUS_COLOR[status] ?? "#374151",
  whiteSpace: "nowrap",
});
      ...
      <table style={tblStyle}>
        <thead style={{ background: "var(--color-surface-raised)" }}>
          <tr>
            ...
            {colVisibility["name"] && (
              <th style={{ ...thStyle, width: 150 }}>Name</th>
            )}
            {colVisibility["email"] && (
              <th style={{ ...thStyle, width: 210 }}>Email</th>
            )}
            {colVisibility["engagement"] && (
              <th style={{ ...thStyle, width: 160 }}>Engagement</th>
            )}
        ...
        <tbody>
          {filtered.length === 0 ? (
            <tr>
              <td
                colSpan={Object.values(colVisibility).filter(Boolean).length}
                style={{
                  ...tdStyle,
                  textAlign: "center",
                  color: "var(--color-on-surface-muted)",
            ...
            filtered.map((row) => (
              ...
              >
                ...
                {colVisibility["status"] && (
                  <td style={tdStyle}>
                    <span style={pillStyle(row.status)}>
