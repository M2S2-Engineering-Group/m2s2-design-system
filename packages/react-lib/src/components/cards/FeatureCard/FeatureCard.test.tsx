...
describe('FeatureCard', () => {
  it('renders the title', () => {
    render(<FeatureCard config={makeFeatureCardConfig({ title: 'Fast Delivery' })} />);
    expect(screen.getByRole('heading', { name: 'Fast Delivery' })).toBeInTheDocument();
  });

  it('renders the body', () => {
    render(<FeatureCard config={makeFeatureCardConfig({ body: 'Ships in 24 hours.' })} />);
    expect(screen.getByText('Ships in 24 hours.')).toBeInTheDocument();
  });

  it('renders items list when items are provided', () => {
    render(<FeatureCard config={makeFeatureCardConfig({ items: ['Item A', 'Item B', 'Item C'] })} />);
  ...
  it('does not render a list when items is absent', () => {
    render(<FeatureCard config={makeFeatureCardConfig({ items: undefined })} />);
  ...
  it('renders the note when provided', () => {
    render(<FeatureCard config={makeFeatureCardConfig({ note: 'Limited time offer.' })} />);
