...
describe('SubscribeForm — anon mode', () => {
  ...
  it('calls subscribeAnon with trimmed email and name on submit', async () => {
    ...
    await user.type(screen.getByPlaceholderText('Your name (optional)'), 'Jane');
    await user.type(screen.getByPlaceholderText('your@email.com'), 'jane@example.com');
    await user.click(screen.getByRole('button', { name: 'Subscribe' }));
    await waitFor(() => expect(subscribeAnon).toHaveBeenCalledWith('jane@example.com', 'Jane'));
  });
...
describe('SubscribeForm — auth mode', () => {
  ...
  it('calls subscribeAuth when subscribe is clicked', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /subscribe to blog updates/i }));
  ...
  it('calls unsubscribeAuth when unsubscribe is clicked', async () => {
    ...
    const user = userEvent.setup();
    render(<SubscribeForm mode="auth" subscribeAuth={subscribeAuth} unsubscribeAuth={unsubscribeAuth} />);
    await user.click(screen.getByRole('button', { name: /subscribe to blog updates/i }));
