...
export function Navbar({ config, loggedIn = false, children }: NavbarProps) {
  ...
  useEffect(() => {
    function onResize() { if (window.innerWidth > 991) setMobile(false); }
    ...
    <>
      <header className={cls}>
        <a className="navbar-brand" href={config.brandPath}>
          {config.brandLogo
            ? <img src={config.brandLogo} alt={config.brand} className="navbar-brand-logo" />
        ...
        <nav className="navbar-desktop" aria-label="Main navigation">
          {navButtons.filter(b => isVisible(b)).map(btn => (
            <a
              key={btn.id}
          ...
          {navDropdowns.filter(b => isVisible(b)).map(btn => (
            <DropdownMenu.Root key={btn.id}>
              <DropdownMenu.Trigger asChild>
                <button className="navbar-nav-btn navbar-nav-btn--dropdown">
                  {btn.title} <span className="navbar-chevron" aria-hidden>▾</span>
                </button>
              </DropdownMenu.Trigger>
              <DropdownMenu.Portal>
                <DropdownMenu.Content className="navbar-dropdown-content" align="end" sideOffset={4}>
            ...
            <DropdownMenu.Trigger asChild>
              <button className="navbar-icon-btn" aria-label="Account menu">
                {config.loginButton.profileImageUrl
                  ? <img src={config.loginButton.profileImageUrl} alt="Profile" className="navbar-avatar" />
                  : <AccountIcon />}
              </button>
            </DropdownMenu.Trigger>
            <DropdownMenu.Portal>
              <DropdownMenu.Content className="navbar-dropdown-content" align="end" sideOffset={4}>
                {config.loginButton.userName && loggedIn && (
                  <div className="navbar-dropdown-user">{config.loginButton.userName}</div>
                )}
                {config.loginButton.dropdownItems
                  .filter(item => isVisible(item))
                  .map(item => <DropdownNavItem key={item.id} item={item} />)}
              </DropdownMenu.Content>
            </DropdownMenu.Portal>
          </DropdownMenu.Root>
        )}
      ...
      {mobileOpen && (
        <div className="navbar-mobile-menu" role="navigation" aria-label="Mobile navigation">
          {config.buttons.filter(b => isVisible(b)).flatMap(btn => {
            if (btn.isDropdown) {
              return (btn.dropdownItems ?? [])
                .filter(item => isVisible(item))
                .map(item => <MobileNavItem key={item.id} item={item} onClose={() => setMobile(false)} />);
            }
            return [
              <a
                key={btn.id}
...
function MobileNavItem({ item, onClose }: { item: DropdownItem; onClose: () => void }) {
  if ('href' in item) {
    const a = item as AnchorDropdownItem;
    return (
      <a
  ...
  if ('onClick' in item) {
    const c = item as ClickableDropdownItem;
    return (
      <button className="navbar-mobile-link" onClick={() => { c.onClick(); onClose(); }}>
        <ItemContent item={item} />
      </button>
    );
  }
...
function ItemContent({ item }: { item: DropdownItem }) {
  return (
    <>
      {item.imgSrc && <img src={item.imgSrc} alt="" className="navbar-dropdown-img" />}
      {item.text}
    </>
  );
}
...
function AccountIcon() {
  return (
    <svg className="navbar-account-icon" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
