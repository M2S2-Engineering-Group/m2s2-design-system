...
function BrandConfigurator(): React.JSX.Element {
  ...
  function onGradientChange(token: typeof gradientTokens[0], stop: 'start' | 'end', color: string): void {
    const newStops = { ...stops, [stop === 'start' ? token.startKey : token.endKey]: color };
    setStops(newStops);
  ...
  function applyPreset(key: string): void {
    ...
    setStops(extractGradientStops(gradientTokens, map as Record<keyof object, string>));
  }
      ...
      <header className="bc-header">
        <div>
          ...
          <p className="bc-subtitle">
            ...
            Changes apply live to the entire Storybook instance.
          </p>
        </div>
        <div className="bc-presets">
          ...
          {THEMES.map(theme => (
            ...
            >
              <span className="bc-preset-swatch" style={{ background: theme.gradientBrand135 }} />
              {theme.label}
            </button>
          ))}
          <button className="bc-preset-btn bc-preset-btn--reset" onClick={reset}>Reset</button>
        </div>
      </header>
              ...
              {tokens.map(t => (
                <div key={t.cssVar} className="bc-token-row">
                  <div className="bc-swatch-wrap">
                    <input type="color" className="bc-picker" value={getValue(t.cssVar)}
                           onChange={e => setVar(t.cssVar, e.target.value)} />
                    <div className="bc-swatch" style={{ background: getValue(t.cssVar) }} />
                  </div>
              ...
              <div key={g.cssVar} className="bc-gradient-row">
                <div className="bc-gradient-stops">
                  <div className="bc-stop-wrap">
                    ...
                    <input type="color" className="bc-picker bc-picker--sm"
                           value={stops[g.startKey]}
                           onChange={e => onGradientChange(g, 'start', e.target.value)} />
                  </div>
                  <div className="bc-gradient-preview" style={{ background: getValue(g.cssVar) }} />
                  <div className="bc-stop-wrap">
                    <span className="bc-stop-label">End</span>
                    <input type="color" className="bc-picker bc-picker--sm"
                           value={stops[g.endKey]}
                           onChange={e => onGradientChange(g, 'end', e.target.value)} />
                  </div>
                </div>
                <div className="bc-token-info">
